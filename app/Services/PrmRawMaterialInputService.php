...
class PrmRawMaterialInputService
{
    ...
    public function importHeader(array $header)
    {
        // stok
        // Log::info('log excel import nomor nota internal service ' . $header['doc_no']);
        // $itemObject = (object)$header;
        // Cari item berdasarkan id_box dan nomor_batch
        $existingItem = PrmRawMaterialInput::where('nomor_nota_internal', $header['nomor_nota_internal'])
            // ->where('created_at')
            ->first();
        // Log::info($existingItem);
        // return $existingItem
    ...
    public function importItem(array $item)
    {
        ...
        $dataToUpdate = [
            ...
            'berat_keluar'          => $itemObject->berat_keluar ?? 0,
            'sisa_berat'            => $itemObject->berat_masuk,
            'avg_kadar_air'         => $averageKadarAir,
            'modal'                 => $itemObject->fix_harga_deal,
            'total_modal'           => $itemObject->fix_harga_deal * $itemObject->berat_bersih,
            'keterangan'            => $itemObject->keterangan,
