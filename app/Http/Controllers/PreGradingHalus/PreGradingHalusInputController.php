...
class PreGradingHalusInputController extends Controller
{
    ...
    public function destroy($nomor_bstb): RedirectResponse
    {
        try {
            ...
            $PreCleaningInputs = PreGradingHalusInput::where('nomor_bstb', '=', $nomor_bstb)->get();
            ...
            foreach ($PreCleaningInputs as $PreCleaningI) {
                ...
                $PreCleaningS = PreGradingHalusStock::where('id_box_grading_kasar', '=', $PreCleaningI->id_box_grading_kasar)
                ...
                if ($PreCleaningS) {
                    ...
                    $stockPrmRawMaterial = TransitPreCleaningStock::where('id_box_grading_kasar', '=', $PreCleaningI->id_box_grading_kasar)
                        ->where('id_box_raw_material', '=', $PreCleaningI->id_box_raw_material)
                        ->first();
                    ...
                    if ($stockPrmRawMaterial) {
                        ...
                        $stockPrmRawMaterial->update([
                            'berat_kirim' => max($beratSebelumnya - $perbedaanBerat, 0),
                            'pcs_kirim' => max($pcsSebelumnya - $perbedaanPcs, 0),
                ...
                if ($stockPrmRawMaterial) {
                    $stockPrmRawMaterial->update([
                        'berat_kirim' => $stockPrmRawMaterial->berat_kirim + $beratSebelumHapus,
                        'pcs_kirim' => $stockPrmRawMaterial->pcs_kirim + $pcsSebelumHapus,
