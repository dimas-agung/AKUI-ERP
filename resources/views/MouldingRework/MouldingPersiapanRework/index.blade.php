        ...
        <div class="card border border-primary border-3">
            <div class="card-header">
                <div class="col-sm-12 d-flex justify-content-between">
                    <h4 class="card-title">Data Moulding Rework Persiapan</h4>
                    <div style="position: absolute;right: 25px;">
                        <a class="btn btn-outline-warning rounded-pill" style="margin-right: 10px" onclick="toggleFilter()">
                            <strong>Filter</strong>
                        </a>
                        <a href="{{ Route('MouldingReworkPersiapan.create') }}" class="btn btn-outline-success rounded-pill">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="form-button-action">
                                            @if ($item->status == 1)
                                                <form style="display: flex" id="deleteForm{{ $item->nomor_job_rework }}"
                                                    action="{{ route('MouldingReworkPersiapan.destroy', $item->nomor_job_rework) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-link btn-danger"
                                                        data-original-title="Remove"
                                                        onclick="confirmDelete('{{ $item->nomor_job_rework }}')">
                            ...
                            @empty
                                <div class="alert alert-danger">
                                    Data Moulding Rework Persiapan belum Tersedia.
                                </div>
                            @endforelse
                        </tbody>
                    </table>
