...
class MouldingPersiapanExtraService
{
    public function store(Request $request)
    {
        ...
        foreach ($dataArray as $key => $data) {
            ...
            if ($validator->fails()) {
                ...
                try {
                    ...
                    $MouldingPersiapanExtra = MouldingPersiapanExtra::create($mergedData);
                    ...
                    $sumTotalModal = $MouldingStock->total_modal_nomor_job + $MouldingPersiapanExtra->total_modal;
                    $sumBerat = $MouldingStock->berat_job + $MouldingPersiapanExtra->berat_job;
                    $modalBaru = $sumTotalModal/$sumBerat;
                    if ($MouldingStock) {
                        // Jika item dengan nomor_job yang sama ada, tambahkan berat_job dan pcs_job
                        $MouldingStock->update([
                            'modal_nomor_job' => $modalBaru,
                            'total_modal_nomor_job' => $sumTotalModal,
                    ...
                    $MouldingPenyebaran = MouldingPenyebaran::where('nomor_job', $itemObject->nomor_job)->first();
                    if ($MouldingPenyebaran) {
                        // Jika item dengan nomor_job yang sama ada, tambahkan berat_job dan pcs_job
                        $MouldingPenyebaran->update([
                            'modal_nomor_job' => $modalBaru,
                            'total_modal_nomor_job' => $sumTotalModal,
                            'berat_job' => $MouldingPenyebaran->berat_job + $itemObject->berat_job,
                            'pcs_job' => $MouldingPenyebaran->pcs_job + $itemObject->pcs_job,
                        ]);
                    } 
                    $MouldingPengembalian = MouldingPengembalian::where('nomor_job', $itemObject->nomor_job)->first();
                    if ($MouldingPengembalian) {
                        // Jika item dengan nomor_job yang sama ada, tambahkan berat_job dan pcs_job
                        $MouldingPengembalian->update([
                            'modal_nomor_job' => $modalBaru,
                            'total_modal_nomor_job' => $sumTotalModal,
                            'berat_job' => $MouldingPengembalian->berat_job + $itemObject->berat_job,
                            'pcs_job' => $MouldingPengembalian->pcs_job + $itemObject->pcs_job,
                        ]);
                    } 
                    ...
                    return response()->json([
                        ...
                        'redirectTo' => route('MouldingPersiapanExtra.create')
                    ], 504);
                }
            }
        }
        ...
        return response()->json([
            ...
            'redirectTo' => route('MouldingPersiapanExtra.index')
        ], 200);
    }
    ...
    public function destroy($nomor_job): RedirectResponse
    {
        try {
            ...
            $MouldingPersiapanExtras = MouldingPersiapanExtra::where('nomor_job', '=', $nomor_job)->get();

            if ($MouldingPersiapanExtras->isEmpty()) {
                // Redirect ke index dengan pesan error jika data tidak ditemukan
                return redirect()->route('MouldingPersiapanExtra.index')->with(['error' => 'Data tidak ditemukan!']);
            }

            foreach ($MouldingPersiapanExtras as $MouldingPersiapanExtra) {
                // Ambil data GradingWarnaStock berdasarkan id_box
                $GradingWarnaStocks = GradingWarnaStock::where('id_box_grading_warna', '=', $MouldingPersiapanExtra->id_box_grading_warna)->get();
                ...
                foreach ($GradingWarnaStocks as $GradingWarnaStock) {
                        ...
                        $perbedaanBerat = $MouldingPersiapanExtra->berat_job;
                        $perbedaanPcs = $MouldingPersiapanExtra->pcs_job;
                        ...
                        $GradingWarnaStock->update([
                            ...
                            'status' => max($MouldingPersiapanExtra->statuss, 1)
                        ]);
                }

                // Update data Gradinwarna
                $GradingWarnas = GradingWarna::where('id_box_grading_warna', $MouldingPersiapanExtra->id_box_grading_warna)->get();
                foreach ($GradingWarnas as $gradingRecord) {
                    $gradingRecord->update(['status' => $MouldingPersiapanExtra->status ?? 1]);
                }
                $MouldingStocks = MouldingStock::where('nomor_job', $MouldingPersiapanExtra->nomor_job)->get();
            ...
            return redirect()->route('MouldingPersiapanExtra.index')->with(['success' => 'Data Berhasil Dihapus!']);
        } catch (\Exception $e) {
            // Rollback transaksi jika terjadi kesalahan
            DB::rollback();

            // Redirect ke index dengan pesan error
            return redirect()->route('MouldingPersiapanExtra.index')->with(['error' => 'Terjadi kesalahan: ' . $e->getMessage()]);
        }
    }
