...
class MouldingPengembalianService
{
    public function store(Request $request)
    {
        ...
        foreach ($dataArray as $data) {
            ...
            if ($validator->fails()) {
                ...
                try {
                    ...
                    MouldingPengembalian::create($mergedData);
                    ...
                    TransitMoulding::create([
                        ...
                        'modal_nomor_job'           => $mergedData['modal_nomor_job'],
                        'total_modal_nomor_job'     => $mergedData['total_modal_nomor_job'],
                    ...
                    $MouldingStock = MouldingStock::where('nomor_job', $MouldingPengembalian->nomor_job)
                        ->get();
                    ...
                    // Ambil semua item yang sesuai dengan kriteria
                    $MouldingPenyebaran = MouldingPenyebaran::where('nomor_job', $MouldingPengembalian->nomor_job)
                        ->get();
                    ...
                    return response()->json([
                        ...
                        'redirectTo' => route('MouldingPengembalian.create')
                    ], 504);
                }
            }
        }
        ...
        return response()->json([
            ...
            'redirectTo' => route('MouldingPengembalian.index')
        ], 201);
    }
    ...
    public function destroy($nomor_job)
    {
        try {
            ...
            $MouldingPengembalian = MouldingPengembalian::where('nomor_job', '=', $nomor_job)->get();

            if ($MouldingPengembalian->isEmpty()) {
                // Redirect ke index dengan pesan error jika data tidak ditemukan
                return redirect()->route('MouldingPengembalian.index')->with(['error' => 'Data tidak ditemukan!']);
            }

            foreach ($MouldingPengembalian as $mouldingPengembalian) {
                // Ambil data TransitMoulding berdasarkan nomor job dan nomor bstb
                $TransitMoulding = TransitMoulding::where('nomor_job', '=', $mouldingPengembalian->nomor_job)
                    ->first();
            ...
            return redirect()->route('MouldingPengembalian.index')->with(['success' => 'Data Berhasil Dihapus!']);
        } catch (\Exception $e) {
            // Rollback transaksi jika terjadi kesalahan
            DB::rollback();

            // Redirect ke index dengan pesan error
            return redirect()->route('MouldingPengembalian.index')->with(['error' => 'Terjadi kesalahan: ' . $e->getMessage()]);
        }
    }
}
