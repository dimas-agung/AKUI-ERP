                        ...
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                            <div class="modal-content">
                                <div class="modal-header bg-primary">
                                    <h4 class="modal-title white" id="myModalLabel33">Input Data Master Batch
                                    </h4>
                                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                        <i data-feather="x"></i>
                                    </button>
                                </div>
                                <form action="{{ route('MasterBatch.store') }}" method="POST">
                                    <div class="modal-body">
                                        @csrf
                                        <label><strong>Nomor Batch</strong></label>
                                        <div class="form-group">
                                            <input type="text" name="nomor_batch" placeholder="Masukkan Nomor Batch"
                                                class="form-control" required
                                                oninvalid="this.setCustomValidity('Mohon isi nomor_batch')"
                                                oninput="this.setCustomValidity('')" value="{{ old('nomor_batch') }}">
                                            <!-- error message -->
                                            @error('nomor_batch')
                                ...
                                <tbody>
                                            ...
                                            <td class="text-center">
                                                <div class="form-button-action">
                                                    <form style="display: flex" id="deleteForm{{ $item->id }}"
                                                        action="{{ route('MasterBatch.destroy', $item->id) }}"
                                                        method="POST">
                                                        <a href="{{ route('MasterBatch.edit', $item->id) }}"
                                    ...
                                    @empty
                                        <div class="alert alert-danger">
                                            Data Master Batch belum Tersedia.
                                        </div>
                                    @endforelse
                                </tbody>
                            </table>
