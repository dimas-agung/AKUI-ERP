...
@endsection
@section('title')
    Grading Kasar Input Adjustment
@endsection
@section('content')
    <div class="col-md-12">
        <div class="card mt-2 border border-primary border-3">
            <div class="card-header">
                <div class="d-flex align-items-center mb-3">
                    <h4 class="card-title">Input Grading Kasar Adjustment</h4>
                </div>
                <hr>
                <form method="POST" class="row g-3" id="myForm">
                    ...
                    <div class="col-md-4">
                        <label for="basic-usage" class="form-label">Pilih Id Box Grading Kasar :</label>
                        <select class="select2 form-select" style="width: 100%;" tabindex="-1" aria-hidden="true"
                            name="id_box_grading_kasar" id="id_box_grading_kasar" placeholder="Pilih Id Box">
                            <option value="">Pilih Id Box</option>
                            @foreach ($GradingKasarStock as $MasterSPRM)
                                {{-- @if ($MasterSPRM->status == 1) --}}
                                    <option value="{{ $MasterSPRM->id_box_grading_kasar }}">
                                        {{ $MasterSPRM->id_box_grading_kasar }}
                                    </option>
                                {{-- @endif --}}
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" class="form-control" id="jenis_grading" readonly>
                        <input type="hidden" name="modal" id="modal">
                        <input type="hidden" name="jenis_raw_material" id="jenis_raw_material">
                        <input type="hidden" name="kadar_air" id="kadar_air">
                        <input type="hidden" name="id_box_raw_material" id="id_box_raw_material">
                    </div>
                    <div class="col-md-3">
...
@section('script')
    <script>
        ...
        $('#id_box_grading_kasar').on('change', function() {
            ...
            $.ajax({
                url: `{{ route('GradingKasarAdjustment.getDataStock') }}`,
                method: 'GET',
                data: {
                    id_box_grading_kasar: selectedbox
                },
    ...
    <script>
        ...
        function addRow() {
            ...
            berat_adjustment.trim() === '' || nama_supplier.trim() === '' || jenis_grading.trim() === '' ||
            ...
            $('#id_box_grading_kasar').val($('#jenis_grading option:first').val()).trigger('change');
        ...
        function simpanData() {
            ...
            $.ajax({
                url: `{{ route('GradingKasarAdjustment.store') }}`,
                method: 'POST',
                data: {
