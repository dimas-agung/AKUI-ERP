        ...
        <div class="card border border-primary border-3">
            <div class="card-header">
                <div class="col-sm-12 d-flex justify-content-between">
                    ...
                    <a href="{{ Route('InputHcrKotor.create') }}" class="btn btn-outline-success rounded-pill">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="form-button-action">
                                            @if ($item->status == 1)
                                                <form style="display: flex" id="deleteForm{{ $item->jenis_hcr_kotor }}"
                                                    action="{{ route('InputHcrKotor.destroy', $item->jenis_hcr_kotor) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-link btn-danger"
                                                        data-original-title="Remove"
                                                        onclick="confirmDelete('{{ $item->jenis_hcr_kotor }}')">
